...
```
Check and see if some items are the same across time at the item level
```{r}

length(datPrePostSec3Base$Sec3Qa.x)
...
datPrePostRandom$Gender = ifelse(datPrePostRandom$Gender == 1,1,0)
datPrePostRandom$Race = ifelse(datPrePostRandom$Race == 5, 0, 1)
datPrePostRandom$Edu = ifelse(datPrePostRandom$Edu < 6, 1, 0)
datPrePostRandom = subset(datPrePostRandom, Time == 0)
```
Now see if randomization is different for those in the control versus the two treatments
```{r}
datPrePostRandomLogit = datPrePostRandom

randomTest = multinom(Treatment ~ Age + Gender + Race + Edu ,data = datPrePostRandom)
z <- summary(randomTest)$coefficients/summary(randomTest)$standard.errors
p <- (1 - pnorm(abs(z), 0, 1)) * 2
p
```
So now there is some difference in who is allocated to what group get compMeans and try to compare percentages across these variables.

See what Jennifer says.
```{r}
tableGender = table(datPrePostRandom$Treatment, datPrePostRandom$Gender)
tableGender
prop.table(tableGender)
describe.factor(datPrePostRandom$Treatment)
```
...
compmeans(datPrePostAnalysisTest$Sec2Total, datPrePostAnalysisTest$Time)
# Try centering the assessment variables
datPrePostAnalysisTest$Sec1Total = scale(datPrePostAnalysisTest$Sec1Total, center = TRUE, scale = FALSE)


datPrePostAnalysisTest$Sec2Total = scale(datPrePostAnalysisTest$Sec2Total, center = TRUE, scale = FALSE)

datPrePostAnalysisTest$Sec3Total = scale(datPrePostAnalysisTest$Sec3Total, center = TRUE, scale = FALSE)
...
modelOutcome2 = rlmer(Sec2Total ~ Treatment*Time + (1 | ID),  data = datPrePostAnalysisTest)
...
Extra
```{r}
### Try without time with nlme
modelOutcome1 = lme(Sec2BaseTotal ~ Treatment, random =~1 | ID, data = datPrePostAnalysisTest)
summary(modelOutcome1)

# Try without treatment
modelOutcome1 = lme(Sec2BaseTotal ~ Time, random =~1 | ID, data = datPrePostAnalysisTest)
summary(modelOutcome1)



# Try with a different outcomes
modelOutcome1 = lmer(Age ~ Treatment*Time + (1 | ID),  data = datPrePostAnalysisTest)
summary(modelOutcome1)

# Try without treatment
modelOutcome1 = lmer(Age ~ Race + (1 | ID),  data = datPrePostAnalysisComplete)
summary(modelOutcome1)


summary(datPrePostAnalysisComplete)

modelOutcome1 = lmer(Sec2BaseTotal ~ Time + (1 | ID),  data = datPrePostAnalysisComplete)

### Try nlme, time is the problem.  

modelOutcome1 = lme(Sec2BaseTotal ~ Treatment, random =~ 1 | ID,  data = datPrePostAnalysisComplete)

write.csv(datPrePostAnalysisComplete, "datPrePostAnalysisComplete.csv", row.names = FALSE)
summary(modelOutcome1)

compmeans(datPrePostAnalysisComplete$Sec1BaseTotal, datPrePostAnalysisComplete$Time)
length(datPrePostAnalysisComplete$Time)


## Why is this not even???  No if there is someone missing at time point 1, but not 0, 0 will stay, because it only deletes the one person.
describe.factor(datPrePostAnalysisComplete$Time)
```


The problem does not appear to be missing data, or writing and uploading as csv or with one treatment category being much smaller than the other

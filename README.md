...
Race translation
Unfortunately werid stuff is happending and changes the codes, but here is the translation
12 == 5
10 == 3
1 == " " change to NA
13 == 8
...
# For race descriptives just come here

# Need to create a multiracial category, which will be 8 just use a bunch of ifelse statements.  This makes things funky see code above

datPrePost$RaceTest = ifelse(datPrePost$Race == '1, 3', 8, datPrePost$Race)
describe.factor(datPrePost$RaceTest)
describe.factor(datPrePost$Race)

#Getting all the multiracial categories the same. See translation above.

# If above 5 on original scale then other
...
datPrePostAnalysis = data.frame(ID = datPrePost$ID, Treatment = datPrePost$Treatment.x, Age =  datPrePost$Age, Gender = datPrePost$Gender, Race = datPrePost$Race, Edu = datPrePost$Edu, Time = datPrePost$time, Sec1BaseTotal, Sec2BaseTotal, Sec3BaseTotal)

describe(datPrePostAnalysis)
datPrePostAnalysisComplete = na.omit(datPrePostAnalysis)
dim(datPrePostAnalysisComplete)[1]/ dim(datPrePostAnalysis)[1]
